...
class PointTransformerSeg(nn.Module):
    ...
    def forward(self, x, idx1 = None, idx2 = None):
        ...
        heatmap=self.H_fc3(points).sigmoid()
        heatmap=heatmap.view(points.size(0),points.size(1),heatmap.size(2),1)
        offset = self.O_fc3(points)
        offset = offset.view(points.size(0),points.size(1),heatmap.size(2),3)

        output=torch.cat((heatmap, offset), 3)
